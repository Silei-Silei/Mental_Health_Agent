...
def invoke_api(
    ...
    if not base_url:
        raise ValueError(
            "API base URL is required. Pass --api-base or set API_BASE env var."
        )
...
def main():
    ...
    chat_p.add_argument(
        "--conversation-history", help="JSON string of conversation history"
    )
    chat_p.add_argument("--mood-context", help="JSON string of current mood context")
    chat_p.add_argument(
        "--api-base",
        default=os.environ.get("API_BASE"),
        help="Base URL like https://abc.execute-api.us-east-1.amazonaws.com/prod",
    )
    ...
    checkin_p.add_argument(
        ...
        default="custom",
        help="Type of check-in",
    )
    checkin_p.add_argument(
        "--mood-rating", type=int, choices=range(1, 11), help="Mood rating 1-10"
    )
    checkin_p.add_argument(
        "--energy-level", type=int, choices=range(1, 11), help="Energy level 1-10"
    )
    checkin_p.add_argument(
        "--sleep-quality", type=int, choices=range(1, 11), help="Sleep quality 1-10"
    )
    checkin_p.add_argument(
        "--stress-level", type=int, choices=range(1, 11), help="Stress level 1-10"
    )
    checkin_p.add_argument(
        "--anxiety-level", type=int, choices=range(1, 11), help="Anxiety level 1-10"
    )
    checkin_p.add_argument(
        "--social-connection",
        type=int,
        choices=range(1, 11),
        help="Social connection 1-10",
    )
    checkin_p.add_argument(
        "--productivity", type=int, choices=range(1, 11), help="Productivity level 1-10"
    )
    ...
    checkin_p.add_argument(
        "--additional-notes", help="Any additional thoughts or feelings"
    )
    checkin_p.add_argument(
        "--api-base", default=os.environ.get("API_BASE"), help="Base URL"
    )
    ...
    eval_p.add_argument(
        "--evaluation-period",
        choices=["week", "month", "quarter", "custom"],
        default="week",
        help="Time period for evaluation",
    )
    eval_p.add_argument(
        "--include-chat-analysis",
        action="store_true",
        default=True,
        help="Include chat conversation analysis",
    )
    eval_p.add_argument(
        "--include-checkin-data",
        action="store_true",
        default=True,
        help="Include check-in questionnaire data",
    )
    eval_p.add_argument(
        "--api-base", default=os.environ.get("API_BASE"), help="Base URL"
    )
    ...
    rec_p = sub.add_parser(
        "recommendations", help="Get personalized mental health recommendations"
    )
    ...
    rec_p.add_argument(
        "--content-types",
        nargs="+",
        choices=["video", "audio", "text", "interactive"],
        help="Preferred content types",
    )
    rec_p.add_argument(
        "--duration", choices=["short", "medium", "long"], help="Preferred duration"
    )
    rec_p.add_argument(
        "--activity-level",
        choices=["low", "moderate", "high"],
        help="Preferred activity level",
    )
    ...
    schedule_p.add_argument(
        "--mental-health-score", type=float, help="Current mental health score 0-100"
    )
    schedule_p.add_argument(
        "--risk-level",
    ...
    schedule_p.add_argument(
        "--preferred-checkin-times",
        nargs="+",
        help="Preferred check-in times (HH:MM format)",
    )
    schedule_p.add_argument(
        "--max-checkins-per-week", type=int, help="Maximum check-ins per week"
    )
    schedule_p.add_argument(
        "--notification-methods",
        nargs="+",
        choices=["push", "email", "sms"],
        help="Notification methods",
    )
    schedule_p.add_argument(
        "--last-checkin-date", help="Date of last check-in (ISO format)"
    )
    schedule_p.add_argument(
        "--api-base", default=os.environ.get("API_BASE"), help="Base URL"
    )
    ...
    profile_p.add_argument(
        ...
        default="get_profile",
        help="Profile action to perform",
    )
    profile_p.add_argument(
        "--context-type",
        choices=["chat", "recommendations", "evaluation"],
        help="Context type for get_context action",
    )
    ...
    profile_p.add_argument(
        "--preferred-language", help="Preferred language for demographics update"
    )
    profile_p.add_argument(
        "--api-base", default=os.environ.get("API_BASE"), help="Base URL"
    )
    ...
    agent_p.add_argument(
        ...
        help="Bedrock Agent ID (or set AGENT_ID)",
    )
    agent_p.add_argument(
        "--agent-alias-id",
        default=os.environ.get("AGENT_ALIAS_ID"),
        help="Bedrock Agent alias ID (or set AGENT_ALIAS_ID)",
    )
    agent_p.add_argument(
        "--save-s3", action="store_true", help="Save Agent response to S3"
    )

    # Simple Bedrock chat (for testing)
    bedrock_p = sub.add_parser("bedrock-chat", help="Direct Bedrock chat (for testing)")
    bedrock_p.add_argument(
        "--message",
        default="Hello, I'm feeling a bit stressed today.",
        help="Message to send to Bedrock",
    )
    ...
    if args.command == "profile":
        payload = {"user_id": args.user_id, "action": args.action}
    ...
    if args.command == "agent":
        if not args.agent_id or not args.agent_alias_id:
            raise ValueError(
                "Missing Agent identifiers. Provide --agent-id and --agent-alias-id or set AGENT_ID / AGENT_ALIAS_ID env vars."
            )
