...
def handler(event, context):
    ...
    try:
        ...
        message = generate_scheduling_message(
            scheduling_decision, mental_health_score, risk_level
        )

        # Store scheduling decision
        store_scheduling_decision(
            user_id, schedule_id, scheduling_decision, mental_health_score, risk_level
        )
            ...
            "body": json.dumps(
                {
                    ...
                    "notification_scheduled": scheduling_decision[
        ...
        return {
            "statusCode": 500,
            "headers": {"Content-Type": "application/json"},
            "body": json.dumps({"error": "Scheduling failed", "details": str(e)}),
        }
...
def determine_checkin_schedule(
    ...
    if last_checkin_date:
        try:
            last_checkin = datetime.fromisoformat(
                last_checkin_date.replace("Z", "+00:00")
            )
        except:
    ...
    preferred_times = user_preferences.get(
    ...
    if preferred_times:
        ...
        next_checkin_date = next_checkin_date.replace(
...
def generate_scheduling_message(
    scheduling_decision: Dict, mental_health_score: float, risk_level: str
) -> str:
    """
        ...
        history["schedules"].append(
            {
                ...
                "checkin_frequency": schedule_data["scheduling_decision"][
                    "checkin_frequency"
                ],
                "next_checkin_date": schedule_data["scheduling_decision"][
                    "next_checkin_date"
                ],
            }
        )
...
def adjust_schedule_for_user_patterns(
    scheduling_decision: Dict, user_patterns: Dict
) -> Dict[str, Any]:
    """
    ...
    if pattern == "highly_consistent" and consistency >= 0.8:
        ...
        if current_frequency == "daily":
            ...
            scheduling_decision[
                "reasoning"
            ] += " Since you've been very consistent with check-ins, I'll adjust to every 2 days."
        ...
        if current_frequency in ["weekly", "every_3_days"]:
            ...
            scheduling_decision[
                "reasoning"
            ] += " I'll check in more frequently to help you stay consistent with your mental health routine."
