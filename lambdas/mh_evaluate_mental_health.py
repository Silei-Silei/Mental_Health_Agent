...
def collect_evaluation_data(user_id: str, evaluation_period: str, include_chat_analysis: bool, include_checkin_data: bool) -> Dict[str, Any]:
    """
    ...
    if include_chat_analysis:
        ...
        evaluation_data["conversation_moods"] = [conv.get("mood_detected", "neutral") for conv in chat_data]
        if not evaluation_data["has_data"]:
            evaluation_data["has_data"] = len(chat_data) > 0
    
    return evaluation_data

def get_user_checkin_data(user_id: str, start_date: datetime, end_date: datetime) -> List[Dict]:
    """
...
def get_user_chat_data(user_id: str, start_date: datetime, end_date: datetime) -> List[Dict]:
    """
...
def perform_mental_health_evaluation(evaluation_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    recommendations = generate_evaluation_recommendations(overall_score, risk_factors, strengths)
...
def calculate_overall_score(checkin_data: List[Dict], conversation_moods: List[str]) -> float:
    """
    Calculate overall mental health score (0-100).
    """
    if not checkin_data:
        # Base score on conversation moods if no check-in data
        mood_scores = {"happy": 80, "good": 70, "neutral": 50, "sad": 30, "anxious": 25, "stressed": 20}
        if conversation_moods:
            avg_mood_score = statistics.mean([mood_scores.get(mood, 50) for mood in conversation_moods])
    ...
    if conversation_moods:
        mood_adjustments = {"happy": 5, "good": 2, "neutral": 0, "sad": -5, "anxious": -8, "stressed": -10}
        mood_adjustment = statistics.mean([mood_adjustments.get(mood, 0) for mood in conversation_moods])
...
def assess_stress_level(checkin_data: List[Dict], conversation_moods: List[str]) -> str:
    ...
    stress_scores = [checkin.get("responses", {}).get("stress_level", 5) for checkin in checkin_data]
...
def identify_risk_factors(checkin_data: List[Dict], conversation_moods: List[str]) -> List[str]:
    """
    ...
    # Check for isolation
    if checkin_data:
        social_scores = [checkin.get("responses", {}).get("social_connection", 5) for checkin in checkin_data]
...
def identify_strengths(checkin_data: List[Dict], conversation_moods: List[str]) -> List[str]:
    """
    ...
    if checkin_data:
        ...
        gratitude_responses = [checkin.get("responses", {}).get("gratitude", "") for checkin in checkin_data]
        gratitude_count = sum(1 for g in gratitude_responses if g and len(g.strip()) > 0)
        if gratitude_count >= len(checkin_data) * 0.7:  # 70%+ show gratitude
            strengths.append("gratitude_practice")
        
        # Check for goal setting
        goal_responses = [checkin.get("responses", {}).get("goals", "") for checkin in checkin_data]
...
def generate_evaluation_recommendations(overall_score: float, risk_factors: List[str], strengths: List[str]) -> List[str]:
    """
    ...
    if overall_score < 40:
        ...
        recommendations.append("Focus on basic self-care: sleep, nutrition, and gentle movement")
    ...
    if "high_stress" in risk_factors:
        recommendations.append("Practice daily relaxation techniques like deep breathing or meditation")
    if "poor_sleep" in risk_factors:
        recommendations.append("Establish a consistent sleep routine and bedtime ritual")
    if "social_isolation" in risk_factors:
        recommendations.append("Make an effort to connect with friends or family regularly")
    if "low_mood" in risk_factors:
        recommendations.append("Engage in activities that bring you joy and fulfillment")
    
    # Strength-based recommendations
    if "gratitude_practice" in strengths:
        recommendations.append("Continue your gratitude practice - it's a powerful tool")
...
def store_evaluation_results(user_id: str, evaluation_id: str, evaluation_results: Dict):
    """
