...
# Configuration
API_BASE = os.environ.get("API_BASE", "https://ot173x9io1.execute-api.us-west-2.amazonaws.com")
...
def main():
    st.set_page_config(
        page_title="Mental Health Agent Playground",
        page_icon="🧠",
        layout="wide"
    )
    
    st.title("🧠 Mental Health Agent Playground")
    st.markdown("Test and interact with the Mental Health Chatbot Agent")
    
    # Sidebar for user configuration
    st.sidebar.header("User Configuration")
    user_id = st.sidebar.text_input("User ID", value="demo_user_123", help="Unique identifier for the user")
    
    # Main tabs
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["💬 Chat", "📊 Daily Check-in", "📈 Evaluation", "🎯 Recommendations", "⏰ Schedule"])
    ...
    with tab1:
        ...
        with col1:
            ...
            with st.expander("Mood Context (Optional)"):
                mood_context["current_mood"] = st.selectbox(
                    "Current Mood",
                    ["neutral", "happy", "sad", "anxious", "stressed", "tired", "excited", "calm"],
                    index=0,
                    key="chat_mood_select"
                )
                mood_context["stress_level"] = st.slider("Stress Level (1-10)", 1, 10, 5)
            ...
            if st.button("Send Message", type="primary"):
                if user_message.strip():
                    payload = {
                        ...
                        "mood_context": mood_context if mood_context["current_mood"] != "neutral" or mood_context["stress_level"] != 5 else {}
                    }
                    ...
                    if result:
                        ...
                        if "follow_up_questions" in result:
                            clean_result["follow_up_questions"] = result["follow_up_questions"]
        ...
        with col2:
            ...
            if hasattr(st.session_state, 'quick_message'):
                st.text_area("Selected Message", value=st.session_state.quick_message, disabled=True)
    ...
    with tab2:
        ...
        with col1:
            ...
            # Rating questions
            responses = {}
            responses["mood_rating"] = st.slider("How is your overall mood today? (1-10)", 1, 10, 5)
            responses["energy_level"] = st.slider("How is your energy level? (1-10)", 1, 10, 5)
            responses["sleep_quality"] = st.slider("How was your sleep quality? (1-10)", 1, 10, 5)
            responses["stress_level"] = st.slider("How stressed do you feel? (1-10)", 1, 10, 5)
            responses["anxiety_level"] = st.slider("How anxious do you feel? (1-10)", 1, 10, 5)
            responses["social_connection"] = st.slider("How connected do you feel to others? (1-10)", 1, 10, 5)
            responses["productivity"] = st.slider("How productive do you feel? (1-10)", 1, 10, 5)
    ...
    with tab4:
        ...
        with col1:
            ...
            recommendation_type = st.selectbox(
                "Recommendation Type",
                ["immediate", "daily", "weekly", "stress_relief", "mood_boost", "sleep_aid"],
    ...
    with tab5:
        ...
        with col1:
            ...
            max_checkins_per_week = st.slider(
