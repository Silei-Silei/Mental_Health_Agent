...
def handler(event, context):
    ...
    try:
        ...
        checkin_data = process_checkin_data(user_id, checkin_type, responses, additional_notes)
...
def process_checkin_data(user_id: str, checkin_type: str, responses: Dict, additional_notes: str) -> Dict[str, Any]:
    """
...
def generate_checkin_insights(responses: Dict, checkin_type: str) -> str:
    ...
    if wellness_score >= 7:
        ...
        insights.append("It looks like you're going through a challenging time. Remember, this is temporary.")
    
    # Add specific insights
    if responses.get("gratitude"):
        insights.append(f"It's wonderful that you're grateful for: {responses['gratitude']}")
    
    if responses.get("challenges"):
        insights.append("I notice you mentioned some challenges. It's brave to acknowledge them.")
...
def generate_immediate_recommendations(responses: Dict, checkin_type: str) -> List[str]:
    ...
    if "high_stress" in concerns:
        recommendations.append("Try some deep breathing exercises or a short meditation")
        recommendations.append("Consider taking a 10-minute break to step away from stressors")
    ...
    if not concerns:
        recommendations.append("Keep up the great work! Continue with your current positive habits")
