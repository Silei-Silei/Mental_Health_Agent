    ...
    "funny_videos": [
        {
            ...
            "type": "video",
            "description": "A collection of hilarious cat videos to brighten your day",
...
def handler(event, context):
    ...
    try:
        ...
        recommendations = generate_personalized_recommendations(
            recommendation_type, current_mood, preferences, urgency_level, user_context
        )
        
        # Generate personalized message
        personalized_message = generate_personalized_message(current_mood, recommendations, user_context)
        
        # Generate follow-up suggestions
        follow_up_suggestions = generate_follow_up_suggestions(recommendation_type, current_mood)
...
def get_user_mental_health_context(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if eval_response.get("Contents"):
            ...
            latest_eval_key = max(eval_response["Contents"], key=lambda x: x["LastModified"])["Key"]
...
def generate_personalized_recommendations(
    ...
    content_categories = get_content_categories_for_needs(recommendation_type, current_mood, urgency_level)
        ...
        if category in CONTENT_DATABASE:
            for content in CONTENT_DATABASE[category]:
                ...
                if content["type"] in preferred_content_types:
                    if preferred_duration == "any" or content["duration"] in duration_map.get(preferred_duration, []):
                        # Add priority boost based on urgency and mood match
                        priority_score = calculate_priority_score(content, current_mood, urgency_level, user_context)
...
def get_content_categories_for_needs(recommendation_type: str, current_mood: str, urgency_level: str) -> List[str]:
    """
...
def calculate_priority_score(content: Dict, current_mood: str, urgency_level: str, user_context: Dict) -> int:
    """
    ...
    # Urgency boost
    if urgency_level == "high" and content.get("duration") in ["3 minutes", "5 minutes"]:
        score += 2
...
def generate_personalized_message(current_mood: str, recommendations: List[Dict], user_context: Dict) -> str:
    """
    ...
    if recommendations:
        ...
        if top_rec.get("type") == "video" and "funny" in top_rec.get("title", "").lower():
            base_message += " Sometimes a good laugh is exactly what we need!"
        elif "breathing" in top_rec.get("title", "").lower():
            base_message += " Deep breathing can be incredibly powerful for calming your mind."
    
    return base_message

def generate_follow_up_suggestions(recommendation_type: str, current_mood: str) -> List[str]:
    """
    ...
    if recommendation_type == "immediate":
        suggestions.append("Try one of these recommendations right now and see how you feel")
        suggestions.append("Set a reminder to check in with yourself in an hour")
    elif recommendation_type == "daily":
        suggestions.append("Consider making one of these activities part of your daily routine")
        suggestions.append("Track how these activities affect your mood over the next few days")
    elif recommendation_type == "stress_relief":
        suggestions.append("Practice these stress-relief techniques regularly, not just when you're stressed")
        suggestions.append("Notice which techniques work best for you and keep them handy")
    ...
    if current_mood in ["sad", "depressed"]:
        ...
        suggestions.append("Remember that these feelings are temporary and you have tools to manage them")
    
    return suggestions[:3]  # Limit to 3 suggestions

def store_recommendations(user_id: str, recommendation_id: str, recommendations: List[Dict], user_context: Dict):
    """
